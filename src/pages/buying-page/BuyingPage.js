    ...
    <div className="buy-outer-container">
      ...
      <div className="buy-container">
        <div className="buy-info-container fade-in">
          ...
          {!tokenDetailsLoading ? (
            tokenSelected === 0 ? (
              <p className="buy-paragraph fade-in">{BUY_INFO_TEXT}</p>
            ) : sellerID === getUserData().profile.stxAddress.testnet ? (
              <p className="buy-paragraph fade-in">{BUY_OWN_TOKEN_TEXT}</p>
            ) : tokenPrice === 0 ? (
              <p className="buy-paragraph fade-in">{CAN_NOT_BUY_TEXT}</p>
            ) : (
              <Table
                labels={['Seller ID', 'Price']}
                values={[sellerID, tokenPrice]}
              />
            )
          ) : (
            <MySpinner />
          ...
          <div className="buy-token-cards-container">
            {!tokensListLoading ? (
              listOfTokens.length > 0 ? (
                ...
                <h1 className="buy-token-cards-container-message fade-in">
                  No tokens available to purchase
                </h1>
              )
            ) : (
              <MySpinner />
